...
public:
	...
	void dijstra(int Ts, int Te) {
		...
		while (1) {
			...
			if (tempV == Te) {
				print_rec(Ts, Te);  //	print   the   PATH   from Ts  to  Te
				cout << endl;
				return;
			}
		}
	...
	void SPFA(int Ts, int Te) {
		...
		while (!queue.empty()){
			...
			list<Edge>::iterator it = lists[tempV].begin();
			while (it != lists[tempV].end()) {
				if (shortestArr[it->toVertex] > shortestArr[tempV] + it->weight) {
					shortestArr[it->toVertex] = shortestArr[tempV] + it->weight;
					ans[it->toVertex] = tempV;
					if (!getMark(it->toVertex)) {
						setMark(it->toVertex);
						queue.push_back(it->toVertex);
					}
				}
				it++;
			}
